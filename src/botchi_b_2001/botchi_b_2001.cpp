...
int main(int argc, char* argv[])
{
	string sLine;
	getline(cin, sLine);
	n32 nCount = SToN32(sLine);
	string sResult;
	for (n32 i = 0; i < nCount; i++)
	{
		getline(cin, sLine);
		n32 nOverlapSize = 0;
		n32 nResultSize = static_cast<n32>(sResult.size());
		n32 nSize = min<n32>(nResultSize, static_cast<n32>(sLine.size()));
		for (n32 j = nSize; j > 0; j--)
		{
			if (memcmp(sResult.c_str() + nResultSize - j, sLine.c_str(), static_cast<size_t>(j)) == 0)
			{
				nOverlapSize = j;
				break;
			}
		}
		sResult.insert(sResult.end(), sLine.begin() + nOverlapSize, sLine.end());
	}
	printf("%s\n", sResult.c_str());
	return 0;
}
